...
//        view.window?.rootViewController = navigationController
        ...
        let closeButton = UIButton(frame: CGRect(x: 320, y: 55, width: 60, height: 30))
        closeButton.setTitle("Close", for: .normal)
        closeButton.backgroundColor = .red
        closeButton.addTarget(self, action: #selector(closeCalendar), for: .touchUpInside)
    ...
    func eventEditViewController(_ controller: EKEventEditViewController, didCompleteWith action: EKEventEditViewAction) { endEventEditing()
        reloadData()
